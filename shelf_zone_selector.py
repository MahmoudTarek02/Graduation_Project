...
class ShelfZoneSelector:
    ...
    def select_live(self, cap: cv2.VideoCapture) -> dict[int, list]:
        self._zones = {}
        self._cur_id = 1
        self._cur_pts = []
        ...
        while True:
            ok, frame = cap.read()
            if not ok:
                print("[ShelfZoneSelector] Live camera frame read failed.")
                self._zones = {}
                break
    ...
    def _finish(self) -> dict[int, list]:
        if self._zones:
    ...
    def _handle_key(self, key: int) -> bool:
        if key == 13:          # Enter — done
            self._save_current()
            return True
        if key == ord("n"):    # next zone (same as right-click)
            self._save_current()
            return False
        if key == ord("z"):    # undo last vertex
            if self._cur_pts:
                self._cur_pts.pop()
                self._redraw()
            return False
